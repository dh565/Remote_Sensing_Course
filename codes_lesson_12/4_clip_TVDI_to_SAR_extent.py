...
TVDI.bounds
minx, miny = SAR.bounds[0], SAR.bounds[1] # copy the coordinates to HERE...
maxx, maxy = SAR.bounds[2], SAR.bounds[3]
bbox = box(minx, miny, maxx, maxy)
print(bbox)

#now we clip the TVDI only for the SAR raster bounds
...
geo = geo.to_crs(crs=SAR.crs.data)
...
# is saved correctly).
epsg_code = int(TVDI.crs.data['init'][5:])
print(epsg_code)
